...
const AddIngredient = ({ ingre, listIngredients }) => {
    ...
    const handleTagClick = (tag) => {
        if (!ingredients.find((ing) => ing.name === tag.name)) {
            setIngredients([
                ...ingredients,
                { ingredient: tag.name, weight: "", id: tag.id },
            ]); // Add ingredient with default weight
        }
    };
    ...
    const handleKeyPress = (e) => {
        if (e.key === "Enter" && searchTerm.trim() !== "") {
            if (
                !ingredients.find((ing) => ing.ingredient === searchTerm.trim())
            ) {
                setIngredients([
                    ...ingredients,
    ...
    const handleRemoveIngredient = (ingredientName) => {
        setIngredients(
            ingredients.filter((ing) => ing.ingredient !== ingredientName),
        );
    };
