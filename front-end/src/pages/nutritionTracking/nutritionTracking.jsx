...
const NutritionTracking = () => {
        ...
        const getAllPlans = async () => {
            try {
                ...
                const response = await fetch(
                    `http://localhost:3000/api/plans/all/${user._id}`,
                    {
                        method: "GET",
                        headers: {
    ...
    useEffect(() => {
        ...
        const matchingPlans = plans.filter((plan) => {
            ...
            return (
                normalizedPickDate.getTime() === normalizedPlanStart.getTime()
            );
        });
        ...
        const caclueCalories = async () => {
            try {
                setIsLoading(true);
                const response = await fetch(
                    "http://localhost:3000/api/plans/calculate",
                    {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                ...
                const data = await response.json();
                if (
                    data.message === "Plan nutrition calculated successfully."
                ) {
    ...
    useEffect(() => {
        ...
        dataTracking.forEach((item) => {
            if (item.label === "calories" && item.value > 50) {
                listWarning.push(
                    "This plan contains too many calories. It may exceed your daily caloric needs.",
                );
            } else if (item.label === "protein" && item.value > 35) {
                listWarning.push(
                    "High protein intake can stress your kidneys. Consider moderating protein consumption.",
                );
            } else if (item.label === "protein" && item.value < 10) {
                listWarning.push(
                    "Low protein intake. Protein is crucial for muscle repair and overall health.",
                );
            } else if (item.label === "fat" && item.value < 20) {
                listWarning.push(
                    "Your fat intake is too low. Fats are essential for brain function and energy.",
                );
            } else if (item.label === "fat" && item.value > 35) {
                listWarning.push(
                    "Fat content is excessively high. This may increase the risk of cardiovascular diseases.",
                );
            } else if (item.label == "carbs" && item.value < 45) {
                listWarning.push(
                    "Your carbohydrate intake is very low. This may cause fatigue and lack of energy.",
                );
            } else if (item.label == "carbs" && item.value > 65) {
                listWarning.push(
                    "Excessive carbohydrate intake can lead to spikes in blood sugar levels. Consider reducing carbs.",
                );
            }
        });
    ...
    const handleAddEvent = async (newEvent) => {
        ...
        const planData = {
            ...
            ingredients:
                newEvent.extendedProps.ingredients.map((item) => ({
                    ingredient: item.id,
    ...
    const handleUpdateEvent = async (updateEvent) => {
        try {
            ...
            const response = await fetch(
                `http://localhost:3000/api/plans/update/${updateEvent.id}`,
                {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json",
    ...
    const handleDeleteEvent = async (deleteEvent) => {
        try {
            setIsLoading(true);
            const response = await fetch(
                `http://localhost:3000/api/plans/delete/${deleteEvent.id}`,
                {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
